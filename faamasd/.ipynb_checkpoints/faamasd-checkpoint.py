...
def create_single_dimension(da):
    """Flatten a (time, spsNN) DataArray into a single time dimension,
    preserving all 2D coordinates that share the same dims."""
    
    if len(da.dims) == 2:
        time_dim, sps_dim = da.dims

        # Extract numeric frequency from e.g. 'sps32'
        m = re.search(r'\d+', sps_dim)
        if m is None:
            raise ValueError(f"Cannot extract frequency from dimension name '{sps_dim}'")
        freq = int(m.group())

        # Stack into one dimension and extract levels
        stacked = da.stack(new_dim=(time_dim, sps_dim))
        mi = stacked['new_dim'].to_index()
        time_level = np.asarray(mi.get_level_values(0), dtype=float)
        sps_level  = np.asarray(mi.get_level_values(1), dtype=float)

        # Compute new continuous time coordinate
        time_coord = time_level + sps_level / float(freq)

        # Flatten data values
        flat_vals = stacked.values.ravel(order='C')

        # Build coordinates dict — start with new time
        coords = {'time': ('time', time_coord)}

        # Add flattened 2D coords that share same dims
        for cname, c in da.coords.items():
            if c.dims == da.dims:
                coords[cname] = ('time', c.values.ravel(order='C'))
            elif cname != time_dim and c.dims == (time_dim,):
                # keep other 1D coords, but skip the original time coord
                coords[cname] = (time_dim, c.values)

        # Construct flattened DataArray
        new_da = xr.DataArray(
            data=flat_vals,
            coords=coords,
            dims=('time',),
    ...
    # Already 1D — just return unchanged
    return da
